    ...
    class spPSO : PSO
    {
        ...
        public override void InitSwarm()
        {
            for (int i = 0; i < sSwarm.Member; i++)
            {
                ...
                {
                    sSwarm.pParticle[i].Position[j] = Function.lowerP(this.fx) + (Function.upperP(this.fx)-Function.lowerP(this.fx)) * rand.NextDouble();
                    sSwarm.pParticle[i].Velocity[j] = 0;
                    sSwarm.pParticle[i].BestP[j] = sSwarm.pParticle[i].Position[j];
                    sSwarm.pParticle[i].PosMin[j] = Function.lowerP(this.fx);
                    sSwarm.pParticle[i].PosMax[j] = Function.upperP(this.fx);
                }
