...
{	// PSO for minimization problem
    class spPSO : PSO
    {
        ...
        public spPSO(Tour T,int fx,int nPar, int nIter, int nNB, double dwmax, double dwmin, double dcp, double dcg, double dcl, double dcn)
            :
            base(nIter, nNB, dwmax, dwmin, dcp, dcg, dcl, dcn)
        {
            base.SetDimension(nPar, T.NumLocs-1);
            this.fx = fx;
            this.tspTour = T;
        }
	...
	class MainClass
	{
		public static void PSO(out Tour TSPtour, int fx,double[] PSOparas,out double ObjVal,out double[] Avg,out double[] index, out ArrayList Ani,out ArrayList AniS,out Particle Gb)
        {
            #region get input data and create historical data storage
            //read locations from text file
            DataInput.ReadfromFile(out TSPtour);
            TSPtour.Calculate_Matrix_Distance();
            //animation data
            double BestObj=0;
            ArrayList sAni;
            ArrayList sAni2;
			...
			for(int i=0; i<noRep; i++)
			{
                ...
                PSO GlobalSwarm = new spPSO(TSPtour,fx,noPar, noIter, noNB, wMax, wMin, cP, cG, cL, cN);
